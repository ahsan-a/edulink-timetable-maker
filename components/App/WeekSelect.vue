			...
			<div class="card flex-shrink-0 w-full max-w-full shadow-2xl bg-base-200">
				<div class="card-body" v-if="timetableStatus === 'week'">
					...
					<div class="form-control">
						...
						<p class="text-xs italic mt-2"
							>Click the calendar icon to select. If all of your weeks don't appear here in holidays, select a date in the future.
							Ensure your date has all the days needed.</p
						>
					</div>
					<label class="label">
				...
				<div class="card-body" v-if="timetableStatus === 'filter'">
					...
					<p class="text-sm font-medium text-center text-warning mt-2"
						>Please be aware that this is not the final timetable. Customisation will come later. Deselect the boxes next to unwanted
						rows.</p
					>
						...
						<table class="table w-full mb-4" v-for="week in selectedWeeks">
							...
							<tbody>
								...
								<tr v-for="i in week.days[0].periods.length">
									...
									<th v-for="j in week.days.length" class="text-sm">{{
										week.days[j - 1].lessons[i - 1].teaching_group.subject
									}}</th>
								</tr>
							</tbody></table
						>
